...
dictfname = '../data/jk/data_var/ydict.csv';
...
% load data
datafname = '../data/jk/data_var/data.csv';
...
savedata([fname '_data_orig.csv'], data, t2ym)
%% identification
MAlags = 360;
N = length(data.names);
...
switch idscheme
    ...
    case 'sgnm2' % baseline two sign restrictions
        ...
        ss = 1:2;
     case 'unitsgnm2' % baseline two sign restrictions
        shocknames = [{'mon.pol.', 'CBinfo'} mnames(2+1:end) ynames];
        dims = {[1 2]};
        imonpol = 1; inews = 2;
        test_restr = @(irfs)... %% restrictions by shock (i.e. by column):
...
table_irf(irfs_draws, 1, 10, varnames, [0.1 0.5 0.9]);
